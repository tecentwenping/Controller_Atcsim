...
public:
	...
	void SetBeaconCode(string strCode);
	void SetBeaconName(string strBeaconName);
...
//update by jzh 现在航路采用雷达的结构
///*--------------------航路上的导航台-------------------*/
//class CourseBeacon : public DataObj
//{
//public:
//	CourseBeacon(void);
//	~CourseBeacon(void);
//
//	//拷贝、赋值函数
//	CourseBeacon(const CourseBeacon& courseBeacon);
//	CourseBeacon& operator=(const CourseBeacon& courseBeacon);
//
//
//	//Set方法
//	void SetCourseID(int iCourseID);
//	void SetBeaconSequence(int iSeq);
//	void SetBeaconID(int iBeaconID);
//	void SetAltitude(double dAltitude);
//	void SetIsQietai(bool bIsQietai);
//
//	//Get方法
//	int GetCourseID();
//	int GetBeaconSequence();
//	int GetBeaconID();
//	double GetAltitude();
//	bool GetIsQietai();
//
...
//	Archive& serialize(Archive& ar,const unsigned int version)
//	{
//		return ar & m_iCourseID & m_iSequence & m_iBeaconID & m_dAltitude & m_bIsQietai;
//	}
//
//
//private:
//	int m_iCourseID;		/*航线ID*/
//	int m_iSequence;		/*导航台的序号*/
//	int m_iBeaconID;		/*导航台ID*/
//	double m_dAltitude;	    /*高度*/
//	bool m_bIsQietai;		/*飞机经过导航台时是使用过台还是切台的方式,0：切台，1：过台 default 0*/
//
//};
//
//
...
//	//Set 方法
//	void SetCourseID(int iCourseID);
//	void SetCourseName(std::string strCourseName);
//	void SetDepartureAirportID(int iDepartureAiportID);
//	void SetArrivalAirportID(int iArrivalAirportID);
//	void SetFlyInfo(std::string strFlyInfo);
//
//	//Get方法
//	int GetCourseID();
...
//	std::string GetFlyInfo();
//	std::vector<boost::shared_ptr<CourseBeacon> >& GetCourseBeacon();
//
//	//序列化方法
//	void ReadFromBuffer(const char* pBuffer, int &iOffSize);
//	void WriteToBuffer(char* pBuffer, int &iOffSize);
...
//	Archive& serialize(Archive& ar,const unsigned int version)
//	{
//		return ar & m_iCourseID & m_strCourseName & m_iDepartureAirportID & m_iArrivalAirportID & m_strFlyInfo & m_vecCourseBeacons;
//	}
//
//
//private:
//	int m_iCourseID;				/*航线ID*/
//	std::string m_strCourseName;	/*航线的中文说明*/
//	int m_iDepartureAirportID;	    /*出发机场ID*/
//	int m_iArrivalAirportID;		/*到达机场ID*/
//	std::string m_strFlyInfo;		/*飞行信息*/
//
//	std::vector<boost::shared_ptr<CourseBeacon> > m_vecCourseBeacons; /*航线上的导航台*/
//
//};
...
public:
	...
	vector<boost::shared_ptr<PointPosition> > m_vStcaAreaPoint;//点组成的区域
	template<typename Archive>
	Archive& serialize(Archive& ar,const unsigned int version)
	{
...
public:
	...
	vector<boost::shared_ptr<PointPosition> > m_vMsawAreaPoint;//点组成的区域
...
public:
	...
	int    m_iLineStyle;
	int    m_iColor;
	vector<boost::shared_ptr<PointPosition> > m_vRestrictAreaPoint;//点组成的区域
	template<typename Archive>
	Archive& serialize(Archive& ar,const unsigned int version)
	{
...
public:
	...
	int    m_iLineStyle;
	int    m_iColor;
	vector<boost::shared_ptr<PointPosition> > m_vQnhAreaPoint; //组成qnh区域的点
	template<typename Archive>
	Archive& serialize(Archive& ar,const unsigned int version)
	{
...
public:
	...
	double  m_dLowestAlt;//最低高度
	double  m_dHighestAlt;//最高高度
	vector<boost::shared_ptr<PointPosition> > m_vInhibitoryAreaPoint;//点组成的区域 
};
...
public:
	...
	vector<RoutePt> m_vecRoutePt;     /* ATS包括的航路点 */
};
...
public:
	...
	vector<boost::shared_ptr<PointPosition> > m_vRadarrobePoint; //点组成的区域
};
...
public:
	...
	vector<boost::shared_ptr<RoutePt> > m_vecRoutePt;   /* 航路包括的航路点 */
};
